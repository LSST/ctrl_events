...
PYBIND11_PLUGIN(_eventReceiver) {
    py::module::import("lsst.ctrl.events._receiver");

    py::module mod("_eventReceiver", "Python wrapper for EventReceiver");

    py::class_<EventReceiver, std::shared_ptr<EventReceiver>, Receiver> cls(mod, "EventReceiver");
    ...
    cls.def("getTopicName", &EventReceiver::getTopicName);
    cls.def("getDestinationPropertyName", &EventReceiver::getDestinationPropertyName);
